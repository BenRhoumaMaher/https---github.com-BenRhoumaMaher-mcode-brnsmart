<?php
/* Template Name: recuperer_stagiaires Page */
get_header(); 
...
if ($error_email == '')
{
    ...
    if(!EmailExistePersonneMorale($email)){
        ...
        if ($user=mysqli_fetch_array($result)) {
         ...
         $mail->IsHTML(true);
         $mail->Body = 'Bonjour ' .$user["prenom"]. ' vous trouverez ici le lien pour réinitialiser votre mot de passe http://localhost/brnsmart/reinitialisation_stagiaires?token='.$user["token"];
         if ($mail->Send()) {
