<?php
/* Template Name: recuperer_personne_morale Page */
get_header(); 
...
if ($error_email == '')
{
    ...
    if(!EmailExistePersonnePhysique($email)){
        ...
        if ($user=mysqli_fetch_array($result)) {
            ...
            $mail->IsHTML(true);
            $mail->Body = 'Bonjour ' .$user["prenom"]. ' vous trouverez ici le lien pour réinitialiser votre mot de passe http://localhost/brnsmart/reinitialisation_personne_morale?token='.$user["token"];
            if ($mail->Send()) {
