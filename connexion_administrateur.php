<?php
/* Template Name: connexion_administrateur Page */
get_header(); 
session_start();
?>
<?php
...
if ($error_email == '' || $error_mot_de_passe == '' )
{
    ...
    $activation = ConfirmAccountActiveStatus($email);
    $Profil = Login_AttemptAdmin($email,$mot_de_passe);
    if (!$activation) {
     $msg = "<label class='text-danger'>Veuiller activer votre compte</label>";
        ...
        $_SESSION["idadmin"] = $Profil['id'];
        $_SESSION["prenom"] = $Profil['prenom'];
        $_SESSION["image"] = $Profil['image'];
        $_SESSION["nom"] = $Profil['nom'];
        $_SESSION["email"] = $Profil['email'];
        $_SESSION["telephone"] = $Profil['Telephone'];
        $_SESSION["adresse"] = $Profil['Adresse'];
        $_SESSION["ville"] = $Profil['Ville'];
        $_SESSION["pays"] = $Profil['Pays'];
        $_SESSION["code_postal"] = $Profil['code_postal'];
        $_SESSION["presentation"] = $Profil['Presentation'];
        $_SESSION["utilisateur"] = $Profil['utilisateur'];
        $_SESSION["age"] = $Profil['Age'];
        $_SESSION["specialite"] = $Profil['Specialite'];
        ...
        <div class="container">
            ...
            <div class="signup-form">
                ...
                <form action="" method="post" role="form">
                    ...
                    <span class="req">
                        <a href="http://localhost/brnsmart/recover_account_admin/">Mot de Passe Oubliée ???</a>
                    </span>
                    <div class="form-group">
                        <hr>
