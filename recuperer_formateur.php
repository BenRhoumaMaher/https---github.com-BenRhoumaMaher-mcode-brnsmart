<?php
/* Template Name: recuperer_formateur Page */
get_header(); 
...
if ($error_email == '')
{
    ...
    if(!EmailExisteFormateur($email)){
        ...
        if ($user=mysqli_fetch_array($result)) {
         ...
         $mail->IsHTML(true);
         $mail->Body = 'Bonjour ' .$user["prenom"]. ' vous trouverez ici le lien pour réinitialiser votre mot de passe http://localhost/brnsmart/reinitialisation_formateur?token='.$user["token"];
         if ($mail->Send()) {
